...
- [ ] **`GET /games?playgroup_id=` is N+1**: `ListGamesForPlaygroup` (`internal/games/service.go`) runs one `ListGamePlayers` per game — 2000 queries for a 2000-game account, measured 2026-09-02. Batch it with `game_id = ANY($1)`, same shape as `ListPlayersForGames` (`internal/statistics/query.sql`)
...
- [ ] Dashboard consumes `GET /statistics/dashboard` instead of the per-group `/games` fan-out + `listAllDecks` cursor chain (`pages/index.vue`) — the endpoint is ready, this is the client half. Note it changes one behaviour: `won` becomes the backend's sole-survivor rule (what the statistics screen already uses) instead of the client's "not eliminated"
- [ ] `onMounted(() => refresh())` in `pages/index.vue` re-runs the whole dashboard fetch on hydration: a hard load pays for it twice (SSR + client)
