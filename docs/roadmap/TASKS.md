...
**Última revisión:** 2026-07-26 (auditoría inicial + detalle de auth con Google OAuth + generación de slices playgroups/games/game-actions y fix de tooling sqlc/lint + quality gates de GitHub Actions)
...
- [x] CI/CD con GitHub Actions — quality gates creados en `.github/workflows/`:
  ...
  - Pendiente (no se puede versionar en archivos, requiere el repo ya creado en GitHub): activar **branch protection** en `main` exigiendo que los 3 workflows pasen antes de mergear, y decidir si se limpia `.github/modernize/java-upgrade` (no es de este proyecto)
  - Nota: el job `lint` va a fallar hasta resolver el backlog de 68 issues de golangci-lint mencionado más abajo — es esperado, el gate está exponiendo deuda ya conocida, no una regresión nueva
- [x] `golangci-lint` ejecutable localmente vía Docker — la config `.golangci.yml` (v1) era incompatible con `go 1.25.0` del `go.mod` (ninguna imagen v1.x de golangci-lint, ni compilada desde código fuente, soporta ese toolchain). Se migró la config a formato v2 (`golangci-lint migrate`) y se repinó `Makefile:lint-docker` a `golangci/golangci-lint:v2.12.2` (compilado con go1.26.2). Sigue sin correr en CI (no existe CI todavía).
- [ ] Backlog de lint expuesto ahora que la herramienta corre (68 issues en todo el repo, ninguno en los slices nuevos salvo 2 TODO intencionales): principalmente `revive` (comentarios doc faltantes en símbolos exportados de `users`/`decks`/`statistics`/`sync`/`common`), `godot` (comentarios sin punto final), `gosec` (password hardcodeada en el DSN de desarrollo de `main.go`, log injection), `mnd` (números mágicos), `err113`/`errorlint`. No bloqueante, pero conviene una pasada de limpieza dedicada.
