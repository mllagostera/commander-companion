...
- [ ] **`RecalculateForGame` is additive, not idempotent/re-runnable, and `FinishGame` has no `AND status = 'active'` guard** (found in a 2026-08-01 review, reproduced with a concurrency test — 8 concurrent `FinishGame` calls on one game all succeeded and inflated `games_played`/`games_won` 8x with no way to undo it). **Not yet fixed**: needs (a) a `WHERE status = 'active'` guard on the `FinishGame` UPDATE so only one concurrent call wins, and (b) either a real idempotency key or a genuine recalculate-from-scratch path (the DBML already claims "recalculated if a historical game is modified," which isn't possible today). See DECISIONS-LOG.md and [ADR-0011](../decisions/0011-estrategia-migraciones-y-recalculo-estadisticas.md).
...
- [ ] **Background bulk import of all of a user's decks — scaffold complete, `MoxfieldClient.ListDecksByUsername` still a stub**: `internal/moxfieldimport` (job table, one-active-job-per-user, progress polling) is real and tested against a mocked client, but `StartImport` responds `501` on every real call because the actual "list a Moxfield user's public decks" endpoint was never confirmed (this sandbox's network policy blocks `api2.moxfield.com`). Needs investigating that endpoint from an environment with network access.
