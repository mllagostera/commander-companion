...
- [ ] Friends system: send/accept/reject requests, friends list — distinct from `playgroups` (game groups, not friendship relations) (**phase 1 of 3 done**, 2026-08-15, see [ADR-0017](../decisions/0017-friends-system.md)): backend (`internal/friends`, migration `00017_friend_requests.sql`) + web (`friends.vue`, add by username search only). Still missing: phase 2 (QR generation in `settings.vue`) and phase 3 (Android profile screen, new — Android has none today — plus QR generation and camera scanning), which is the actual "scan to add" flow the feature was requested for.
